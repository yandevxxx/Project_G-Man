...
@section('content')
<div class="mb-5">
    <a href="{{ route('purchases.catalog') }}" class="btn btn-link text-decoration-none p-0 mb-4 small fw-bold text-primary transition-all hover-translate-x">
            ...
            <div class="card border-0 shadow-sm rounded-4 mb-4 overflow-hidden">
                ...
                <div class="card-body p-4">
                    ...
                    <form action="{{ route('purchases.process_checkout') }}" method="POST">
                        ...
                        <div class="mb-4">
                            <label for="quantity" class="form-label small fw-bold text-uppercase ls-wide text-muted mb-2">Quantity</label>
                            <div class="input-group">
                                <button class="btn btn-outline-light border text-muted" type="button" onclick="updateQuantity(-1)"><i class="fas fa-minus"></i></button>
                                <input type="number" class="form-control text-center bg-white border-start-0 border-end-0" id="quantity" name="quantity" value="1" min="1" max="{{ $product->stock }}" readonly>
                                <button class="btn btn-outline-light border text-muted" type="button" onclick="updateQuantity(1)"><i class="fas fa-plus"></i></button>
                            </div>
                            <div class="form-text text-muted text-center mt-2">Available Stock: {{ $product->stock }}</div>
                        </div>
                        ...
                        <div class="d-flex justify-content-between mb-4">
                            ...
                            <span class="h5 fw-bold mb-0 text-primary" id="total-display">Rp {{ number_format($product->price, 0, ',', '.') }}</span>
                        </div>
