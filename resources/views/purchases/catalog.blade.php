        ...
        <div class="col-md-6 col-lg-4 col-xl-3">
            <div class="card h-100 border-0 shadow-sm rounded-4 overflow-hidden transition-all hover-translate-y">
                ...
                <div class="card-body p-4 d-flex flex-column">
                    <div class="mb-2">
                        <span class="badge bg-primary bg-opacity-10 text-primary rounded-pill px-3 py-1 small fw-bold">{{ $product->category->name ?? 'General' }}</span>
                    </div>
                    
                    <h5 class="card-title fw-bold mb-1 text-dark">{{ $product->name }}</h5>
                    <p class="card-text text-muted small mb-3 flex-grow-1">{{ Str::limit($product->description, 60) }}</p>
                    
                    <div class="d-flex justify-content-between align-items-end mt-3">
                        <div>
                            <div class="small text-muted mb-1 text-uppercase fw-bold" style="font-size: 0.7rem; letter-spacing: 0.5px;">Price</div>
                            <div class="h5 mb-0 fw-bold text-dark">Rp {{ number_format($product->price, 0, ',', '.') }}</div>
                        </div>
                        
                        <div class="text-end">
                            @if($product->stock > 0)
                                <a href="{{ route('purchases.checkout', $product->id) }}" class="btn btn-primary rounded-pill px-3 shadow-sm btn-sm">
                ...
                <div class="card-footer bg-white border-0 px-4 pb-4 pt-0">
                    <div class="d-flex align-items-center small text-muted">
                        ...
                        @elseif($product->stock > 0)
                            <span class="text-warning"><i class="fas fa-exclamation-circle me-1"></i> Low Stock: {{ $product->stock }}</span>
                        @else
